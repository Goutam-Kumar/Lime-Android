...
class DotLoader: LinearLayout,LoaderConfigs {
    ...
    private fun initView() {
        ...
        viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                startLoading()
                this@DotLoader.viewTreeObserver.removeOnGlobalLayoutListener(this)
            }
        })
    }
    ...
    override fun initAttributes(attrs: AttributeSet) {
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.DotLoader, 0, 0)

        dotsRadius = typedArray.getDimensionPixelSize(R.styleable.DotLoader_dotloader_dotsRadius, 30)

        useMultipleColors = typedArray.getBoolean(R.styleable.DotLoader_dotloader_useMultipleColors, false)

        if (useMultipleColors) {
            val dotsArrayId = typedArray.getResourceId(R.styleable.DotLoader_dotloader_colorsArray, 0)

            dotsColorsArray = validateColorsArray(dotsArrayId, resources.getColor(android.R.color.darker_gray))

        } else {
            dotsColor = typedArray.getColor(R.styleable.DotLoader_dotloader_dotsColor,
                resources.getColor(android.R.color.darker_gray))
        }

        bigCircleRadius =
            typedArray.getDimensionPixelSize(R.styleable.DotLoader_dotloader_bigCircleRadius, 90)

        animDuration = typedArray.getInt(R.styleable.DotLoader_dotloader_animDur, 2000)
